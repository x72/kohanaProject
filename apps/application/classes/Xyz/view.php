...
class Xyz_View
{
     ...
     * you must call [Xyz_View::set_filename].
     *
     ...
     *
     *     $output = Xyz_View::capture($file, $data);
     *
    ...
    public function capture($kohana_view_filename, array $kohana_view_data)
    {
        ...
        if (Xyz_View::$_global_data) {
            // Import the global view variables to local namespace
            extract(Xyz_View::$_global_data, EXTR_SKIP | EXTR_REFS);
        }
        ...
        try {
            ...
            if (!$isfileto || filemtime($kohana_view_filename) > filemtime($to) || (filesize($to) == 0 && filesize($kohana_view_filename) > 0) || MODE == 1) {
                ...
                $new_content = self::compile_template($file_content);
    ...
    /**
     * Sets a global variable, similar to [Xyz_View::set], except that the
     * variable will be accessible to all views.
     *
     *     Xyz_View::set_global($name, $value);
     *
    ...
    public static function set_global($key, $value = NULL)
    {
        if (is_array($key)) {
            foreach ($key as $key2 => $value) {
                Xyz_View::$_global_data[$key2] = $value;
            }
        } else {
            Xyz_View::$_global_data[$key] = $value;
        }
    }

    /**
     * Assigns a global variable by reference, similar to [Xyz_View::bind], except
     * that the variable will be accessible to all views.
     *
     *     Xyz_View::bind_global($key, $value);
     *
     ...
     * always only be created using [Xyz_View::factory].
     *
     *     $view = new View($file);
     *
     * @param   string $file view filename
    ...
    public function & __get($key)
    {
        ...
        } elseif (array_key_exists($key, Xyz_View::$_global_data)) {
            return Xyz_View::$_global_data[$key];
        } else {
    ...
    /**
     * Magic method, calls [Xyz_View::set] with the same parameters.
     *
     *     $view->foo = 'something';
     *
     * @param   string $key variable name
    ...
    {
        return (isset($this->_data[$key]) OR isset(Xyz_View::$_global_data[$key]));
    }
    ...
    {
        unset($this->_data[$key], Xyz_View::$_global_data[$key]);
        unset(self::$_user_data[$key], Xyz_View::$_global_data[$key]);
    }

    /**
     * Magic method, returns the output of [Xyz_View::render].
     *
     * @return  string
     * @uses    Xyz_View::render
     */
    ...
    public function render($file = NULL)
    {
        ...
        return Xyz_View::capture($this->_file, $this->_data);
    }
    ...
    public function compile_template($str)
    {
        ...
        $str = preg_replace("/\{template\s+([^\}]+)\}/", "<?php echo  Xyz_View::template(\\1);  ?>", $str); //引用模板
        $str = preg_replace("/\{include\s+([^\}]+)\}/", "<?php echo  Xyz_View::template(\\1);  ?>", $str); //引用模板
     ...
     * @param $kohana_view
     * @return string
     * @internal param string $html 匹配到的所有的HTML代码
     */
    ...
    {
        echo Xyz_View::factory($file, self::$_user_data);
    }
